    ...
    async function build_slider_ui() {
      ...
      Object.entries(seriesMap).forEach(([key, label]) => {
        ...
        group.innerHTML = `
          ...
          <input type="range" id="${key}-mult" min="-2" max="2" step="0.01" value="`+calibrations[key].mult+`">
        `;
...
function updateChart() {
  ...
  const calibrated = rawData.flatMap(trace => {
    ...
    // we need to translate the raw key name (raw=data with no calibration applied) to find the 
    // actual data name. A bit of a hack.
    real_key = trace.key.replace("_raw_", "_");

    console.log(trace.key,real_key);
    const cal = calibrations[real_key] || { offset: 0, mult: 1 };
    ...
    const baseTrace = {
      ...
      y: ys,
      mode: "lines",
      name: seriesMap[real_key] || real_key
    };
