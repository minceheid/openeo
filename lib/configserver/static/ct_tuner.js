    ...
    const seriesMap = {
      "eo_current_raw_site":    "Site",
      "eo_current_raw_vehicle": "Vehicle",
      "eo_current_raw_solar":   "Solar"
    };
    ...
    async function build_slider_ui() {
      ...
      calibrations["eo_current_raw_site"].offset=configdata["loadmanagement"].ct_offset_site;
      calibrations["eo_current_raw_site"].mult=configdata["loadmanagement"].ct_calibration_site;
      calibrations["eo_current_raw_vehicle"].offset=configdata["loadmanagement"].ct_offset_vehicle;
      calibrations["eo_current_raw_vehicle"].mult=configdata["loadmanagement"].ct_calibration_vehicle;
      calibrations["eo_current_raw_solar"].offset=configdata["loadmanagement"].ct_offset_solar;
      calibrations["eo_current_raw_solar"].mult=configdata["loadmanagement"].ct_calibration_solar;
    ...
    async function fetchData() {
      try {
        ...
        const url = "getchartdata?type=plotly&"+maxtime+"series=eo_current_raw_vehicle,eo_current_raw_site,eo_current_raw_solar";
        const res = await fetch(url);
        if (!res.ok) throw new Error("HTTP " + res.status);
