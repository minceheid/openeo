...
const statusLocaltime = document.getElementById("statusLocaltime");
...
function angleToTime(angle) {
	...
	minutes = minutes - (minutes % scheduler_granularity);
	//console.log(stat['scheduler']);
...
function saveSchedule(report) {
	...
	fetch('setsettings', {
		...
		body: new URLSearchParams({"scheduler:schedule":JSON.stringify([{ start: start, end: end, amps : currentLimMode['schedule'] }])}),
	})
...
function saveState() {
	...
	fetch('setsettings', {
		...
		body: new URLSearchParams({
			...
			"switch:on": manualOnState,
			"switch:amps": currentLimMode['manual']
		}),
	})
	.then(response => response.json())
	.then(data => {
...
function updateStatus(once) {
	...
	if (once || ((updateTick % updateFreq) == 0)) {
		...
		fetch('getstatus', {
			...
			signal: AbortSignal.timeout(Math.round(statusUpdateInterval*0.85))
		})
		...
		.then(function(stat) {
			...
			statusLocaltime.innerHTML = stat['eo_localtime'];
			...
			if ("openeo_latest_version" in stat && stat["app_version"]!=stat["openeo_latest_version"]) {
					versionInfo.style.color='red';
					versionInfo.innerText="openeo "+stat["app_version"] + " (Update Available)";
					versionInfo.addEventListener("click", function() {window.location.href = "/update.html";});
			}
...
function updateConfig(stat) {
		...
		scheduler_granularity=stat['scheduler']['scheduler_granularity'];
...
function fetchAndUpdateConfig() {
	...
	console.log("configupdateInterval",configUpdateInterval);
	fetch('getconfig', {
		method: 'GET',
		signal: AbortSignal.timeout(Math.round(configUpdateInterval*0.15))
	})
	.then(function(response) { return response.json(); })
...
// window.onload equivalent to allow stacking of multiple events
window.addEventListener("load",function(event) {
	...
	window.setInterval(() => { updateStatus(false); }, statusUpdateInterval);
	window.setInterval(() => { fetchAndUpdateConfig(); }, configUpdateInterval);  // Update the configuration every 30s, in case another user changes it
